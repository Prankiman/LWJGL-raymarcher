...
vec3 light_position = vec3(1.0, 3.0, 3.0);
...
float dist(vec3 p){
	float displacement = sin((5+xx) * p.x) * sin((5+xx) * p.y) * sin((5.0+xx) * p.z) * 0.25;
...
vec4 ray_march(vec3 ro, vec3 rd)
{
	...
	return vec4(0, 0, 0.05, 1);
}


void main() {
	color = ray_march(cam, dir);//vec4(xu, -yu, 0, 1);
    return;
}
