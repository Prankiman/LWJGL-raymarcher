...
vec3 lightColor[3] = vec3[3](vec3(1000), vec3(800), vec3(800, 800, 200));
...
float D_GGX (vec3 N, vec3 H, float roughness){
    ...
    float denom = max((NdotH * NdotH * (a2 - 1.0) + 1.0), 0.0000001);
...
vec4 ray_march(vec3 ro, vec3 rd)
{
    ...
    while(total_distance_traveled < MAXIMUM_TRACE_DISTANCE)
    {
        ...
        if (distance_to_closest < MINIMUM_HIT_DISTANCE) 
        {
            ...
            vec3 idIntensity = (vec3(1) - fresnel_S(max(dot(N, V), 0.0), F0)); 

            indirectDiffuse*= (vec3(1) - metalness)*idIntensity*albedo*ao;
            
            Lo += indirectDiffuse;

            //_____________________________________

            vec3 ambiance = vec3(0.3);

            vec3 ambient = ambiance * albedo * ao;
